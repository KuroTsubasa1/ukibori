...
var Potrace = (function() {
  ...
  // --- Ukibori adapter (added, not part of upstream kilobtye/potrace) ---
  // Trace a binary mask (Int8Array/Uint8Array, 1 = foreground) synchronously and
  // return the traced curves (pathlist[].curve), bypassing the async image load.
  function traceData(data, w, h, params) {
    if (params) setParameter(params);
    bm = new Bitmap(w, h);
    for (var i = 0; i < w * h; i++) bm.data[i] = data[i] ? 1 : 0;
    info.isReady = true;
    pathlist = [];
    bmToPathlist();
    processPath();
    return pathlist.map(function (p) { return p.curve; });
  }
...
if (typeof window !== "undefined") window.Potrace = Potrace;
